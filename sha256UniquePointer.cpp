...
std::unique_ptr<uint8_t[]> padMessage(std::unique_ptr<uint8_t[]> &messageBytes, size_t byteMessageLength){
    if(byteMessageLength <= 55){
        size_t paddedMessageLength = 64; 
        std::unique_ptr <uint8_t []> paddedMessage (new uint8_t[paddedMessageLength]);
        std::copy(messageBytes.get(), messageBytes.get() + byteMessageLength, paddedMessage.get()); 

        paddedMessage[byteMessageLength] = 0x80; 

        std::fill(paddedMessage.get() + byteMessageLength + 1, paddedMessage.get() + 56, 0); 

        // Append message length in bits as big-endian 64-bit integer
        uint64_t messageBits = static_cast<uint64_t>(byteMessageLength) * 8;
        for (int i = 0; i < 8; ++i) {
            paddedMessage[56 + i] = static_cast<uint8_t>((messageBits >> (56 - 8 * i)) & 0xFF);
        }

        // std::cout<<"\nPadded message is: ";
        // for (const auto* it = paddedMessage.get(); it != paddedMessage.get() + 64; ++it) {
        ...
        size_t zeroBytes = (55 - byteMessageLength) % 64;
        if (zeroBytes < 0){
            zeroBytes += 64;
        }
        //std::cout<<"\nZero bytes to be padded = "<<zeroBytes; 
        size_t paddedMessageLength = byteMessageLength + 1 + zeroBytes + 8; 
        size_t numBlocks = paddedMessageLength/64; 
        // std::cout<<"\nLength of original message in bytes for multiple blocks = "<<byteMessageLength;
        // std::cout<<"\nLength of padded message in bytes for multiple blocks = "<<paddedMessageLength; 
        std::unique_ptr <uint8_t []> paddedMessage (new uint8_t[paddedMessageLength]);
        std::copy(messageBytes.get(), messageBytes.get() + byteMessageLength, paddedMessage.get());
        paddedMessage[byteMessageLength] = 0x80; 
        std::fill(paddedMessage.get() + byteMessageLength + 1, paddedMessage.get() + byteMessageLength + 1 + zeroBytes, 0);
        uint64_t messageBits = static_cast<uint64_t>(byteMessageLength) * 8;
        for (int i = 0; i < 8; ++i) {
            paddedMessage[byteMessageLength + 1 + zeroBytes + i] = static_cast<uint8_t>((messageBits >> (byteMessageLength + 1 + zeroBytes - 8 * i)) & 0xFF);
        }

        // std::cout<<"\nPadded message is: ";
        // for (const auto* it = paddedMessage.get(); it != paddedMessage.get() + numBlocks * 64; ++it) {
        //     std::cout << std::hex << static_cast<int>(*it) << " ";
        // }
...
std::array<uint32_t,64> smartWordExpansionSingleBlock(std::unique_ptr<uint8_t []>& paddedMessage){
...
std::array<uint32_t,8> smartComputeSHA256SingleBlock(std::array<uint32_t,64> words, std::array<uint32_t,8> inputHashes){
...
std::vector<std::array<uint32_t,64>> smartWordExpansionMultipleBlocks(std::unique_ptr<uint8_t []>& paddedMessage, int numBlocks){
    ...
    */
    ...
    for(int b = 0; b < numBlocks; b++){
        uint8_t* starttIndex = paddedMessage.get() + b * 64; 
        std::unique_ptr<uint8_t []>singleBlock(new uint8_t[64]);
        std::copy(starttIndex, starttIndex + 64, singleBlock.get()); 

        auto words = smartWordExpansionSingleBlock(singleBlock); 
...
// //Correct output produced for single block of data 
std::array<uint32_t,8> smartComputeSHA256MultipleBlocks(std::vector<std::array<uint32_t,64>> multiWordBlocks, std::array<uint32_t,8> initialHashes){
    /*
    ...
    */
    for(const auto & block: multiWordBlocks){
        ...
        initialHashes = std::move(smartComputeSHA256SingleBlock(block, initialHashes)); 
...
int main(){
    ...
    std::cout << "Time taken for unique pointer to pad: " << durationpad.count() << " milliseconds" << std::endl;
    ...
    if(messageSize <= 55){
        ...
        std::array<uint32_t,8> finalHashSingleBlock = smartComputeSHA256SingleBlock(wordsSingleBlock, initialHashArray); 
        auto end = std::chrono::high_resolution_clock::now();
        std::chrono::duration<double, std::milli> duration = end - start; 
        std::string hexHashSingleBlock = hashToHexUTF8(finalHashSingleBlock);
        std::cout << "\nSHA-256 hash in hex (for unique pointer): " << hexHashSingleBlock << std::endl;
        std::cout << "Execution Time taken (for unique pointer): " << duration.count() << " milliseconds" << std::endl;
    }
    else{
        std::cout<<"\nPreprocessed Message is a multiple data block";
        //int numBlocks = preprocessedMessage.size()/512; 
        std::cout<<"\nPreprocessed Message contains "<<numBlocks<<" blocks"; 
        std::vector<std::array<uint32_t,64>> wordsMultipleBlocks = smartWordExpansionMultipleBlocks(paddedMessage, numBlocks);
        ...
        std::cout << "\nSHA-256 hash in hex (for unique pointer): " << hexHash << std::endl;
        std::cout << "Execution Time taken (for unique pointer): " << duration.count() << " milliseconds" << std::endl;
